...
while running:
    ...
    if endgame == False:
        if paused:
            ...
            screen.fill(GREEN)
            food_char = []
            food_char.append(food[0])
            food_char.append(food[1])
            food_char.append(food_size)
            food_char.append(food_size)

            pygame.draw.rect(screen,WHITE,food_char)
            pygame.draw.rect(screen,WHITE,(0,0,length,50))

            for i in range(len(snake_body)):
            # appending coordinates and sizes of rect and conctantly outputing rect
                nums = []
